...
The video background and story content overlap in one CSS Grid cell. The sticky background keeps its full height and stops at the story boundary; no negative margin is used. This prevents the video from covering The site, project statistics, or the selector after the introduction. Run `npm run stress:hero:bounds -- http://127.0.0.1:4173` to check section boundaries and visibility across six desktop/mobile viewports, scrolling in both directions and resizing the viewport. Add `--webkit` for geometry checks using the installed Playwright WebKit browser; the Windows run uses the poster and fallback fonts.
